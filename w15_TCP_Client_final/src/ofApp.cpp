...
void ofApp::update(){
    ...
    if(tcpClient.isConnected() && vid.isFrameNew() ){
        ...
        //        tcpClient.sendRawMsg(rawBytes, pixelsClient.getTotalBytes());
        
		int size = imgClient.getPixels().getTotalBytes();
		//tcpClient.sendRawMsg((char*)(imgClient.getPixels().getData()), size); 		// Send static image
      
		//send slitscan current line from Camera
        ofPixels line = vid.getPixels().getLine(slitScan.lineNum).asPixels();
		//cout << line.getData() << endl;
		string line_num_msg = 'n' + ofToString(slitScan.lineNum);
        
		bool lineSent = tcpClient.send(line_num_msg);
        
		if (lineSent)
			cout << "sent nLine ";
		else
			cout << "error sending nLine ";
            cout << line_num_msg << " - ";
            int sizeLine = line.getTotalBytes();
            unsigned char* byteLine ;
            byteLine = line.getData();

            bool lineDataSent = tcpClient.sendRawMsg((char*)byteLine, sizeLine);
            if (lineDataSent) {
			cout << "sent lineData ";
		}
		else cout << "error sending lineData ";
            for (int i = 0; i < 6; i++)
			cout << +byteLine[i] << " ";
            cout << endl;
            //delete[] byteLine;
